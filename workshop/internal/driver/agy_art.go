...
// agyStateDir returns agy's mutable state directory. WORKSHOP_AGY_STATE_DIR
// overrides it (tests, relocated installs).
func agyStateDir() (string, error) {
	if v := os.Getenv("WORKSHOP_AGY_STATE_DIR"); v != "" {
		return v, nil
	}
...
func (a *Agy) ListModels(ctx context.Context) ([]string, error) {
	...
	logDir, err := os.MkdirTemp("", "workshop-agy-probe-")
	if err != nil {
		return nil, err
	}
