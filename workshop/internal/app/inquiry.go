...
// "why did the workshop do X?" from the evidence the engine already records —
...
func (a *App) writeInquiryEvidence(ctx context.Context) (string, error) {
	...
	for _, p := range passes {
		...
		if p.LogPath != "" {
			if t := engine.TranscriptArchivePath(p.LogPath); fileExists(t) {
				row.Transcript = t
			}
		}
	...
	doc := map[string]any{
		...
		"note":        "join a commit's Workshop-Pass trailer to `pass` here; transcriptPath is set only where an archived transcript exists",
		"passes":      rows,
