...
func TestCommitAllWithTrailers(t *testing.T) {
	...
	if !strings.Contains(body, "Workshop-Task: ws-123") || !strings.Contains(body, "Workshop-Pass: 7") {
		t.Fatalf("trailers missing:\n%s", body)
	}
	trailerOut := mustGit(t, dir, "log", "-1", "--format=%(trailers:key=Workshop-Task,valueonly)")
...
func TestStatusDirtyAndBranches(t *testing.T) {
	...
	if err := CreateBranch(ctx, dir, "workshop/code", "main"); err != nil {
		t.Fatal(err)
	}
	if !BranchExists(ctx, dir, "workshop/code") || BranchExists(ctx, dir, "nope") {
	...
	n, err := AheadCount(ctx, dir, "workshop/code", "main")
	if err != nil || n != 0 {
...
func TestWorktreeLifecycle(t *testing.T) {
	...
	if err := AddWorktree(ctx, dir, wtPath, "workshop/code", "main"); err != nil {
		t.Fatal(err)
	}
	...
	if err != nil || wt == nil || wt.Branch != "workshop/code" {
	...
	n, err := AheadCount(ctx, dir, "main", "workshop/code")
	if err != nil || n != 1 {
		t.Fatalf("ahead: %d, %v", n, err)
	}
	...
	if err := AddWorktree(ctx, dir, wtPath2, "workshop/art", "main"); err != nil {
		t.Fatal(err)
	}
