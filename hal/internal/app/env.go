...
func (a *App) probeTools(ctx context.Context) []ToolInfo {
	...
	if path, err := driver.ClaudePath(); err != nil {
		claude.Fix = "install Claude Code, or set HAL_CLAUDE_BIN"
	} else {
		claude.Present = true
		claude.Path = path
	...
	if path, err := driver.AgyPath(); err != nil {
		agy.Detail = ""
		agy.Fix = "install the Antigravity CLI (agy), or set HAL_AGY_BIN"
	} else {
		agy.Present = true
		agy.Path = path
	}
	...
	if exe := chroma.CorridorKeyExe(dir); exe != "" {
		...
		ck.Fix = "clone the CorridorKey fork, run its install script, and set [art].corridorkey_dir (or HAL_CORRIDORKEY)"
	}
