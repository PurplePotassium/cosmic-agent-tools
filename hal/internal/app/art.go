...
func (a *App) ArtStatus(ctx context.Context) ArtStatus {
	...
	st.Model, _ = a.Store.GetKV(ctx, artjob.KVArtAgyModel)
	if raw, err := a.Store.GetKV(ctx, artjob.KVArtAgyModels); err == nil && raw != "" {
...
func (a *App) artKeyerOverride(ctx context.Context) []string {
	if raw, err := a.Store.GetKV(ctx, artjob.KVArtRemovers); err == nil && raw != "" {
		var list []string
		if json.Unmarshal([]byte(raw), &list) == nil && len(list) > 0 {
			return list
		}
	}
	if v, err := a.Store.GetKV(ctx, artjob.KVArtRemover); err == nil && v != "" {
		return []string{v}
	}
	return nil
}
...
func (a *App) SetArtKeyers(ctx context.Context, keyers []string) error {
	...
	if err := a.Store.SetKV(ctx, artjob.KVArtRemovers, value); err != nil {
		return err
	}
	// Retire any legacy single-value override so the two keys can never
	// disagree about what "the override" is.
	if err := a.Store.SetKV(ctx, artjob.KVArtRemover, ""); err != nil {
		return err
	}
...
// halt/resume restarts the engine loop within the same process).
...
func (a *App) VerifyArtModelsAsync(ctx context.Context, agyMu *sync.RWMutex) {
	if os.Getenv("HAL_FAKE_BIN") != "" || truthy(os.Getenv("HAL_SKIP_AGY_VERIFY")) {
		return
	}
...
func (a *App) VerifyArtModels(ctx context.Context, agyMu *sync.RWMutex) {
	...
	// (Cross-process runs — `hal doctor`, the user's own agy — are
	...
	_ = a.Store.SetKV(ctx, artjob.KVArtAgyModels, string(raw))
	for _, want := range domain.ArtAgyModels {
		if driver.AgyHasModel(models, want) {
			_ = a.Store.SetKV(ctx, artjob.KVArtAgyModel, want)
	...
	_ = a.Store.SetKV(ctx, artjob.KVArtAgyModel, "")
