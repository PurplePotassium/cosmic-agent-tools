// Package driver abstracts the coding-agent CLIs Hal can spawn. Every
...
type Capabilities struct {
	...
	// Interactive: supports turn-based conversations — one process per turn
	// emitting typed stream-json events on stdout, with cross-process context
	// continuity via a resume flag (InvokeSpec.ResumeSessionID). See
	// docs/interactive-driver.md for the probed contract.
	Interactive bool
}
...
type InvokeSpec struct {
	...
	// Interactive requests a conversational turn: typed stream-json events on
	// stdout ending in a per-turn result. Requires Capabilities.Interactive;
	// non-interactive drivers refuse to Plan it.
	Interactive bool
	// ResumeSessionID continues a prior session in a NEW process with full
	// context (--resume). When set it takes precedence over SessionID (no
	// fresh session is minted).
	ResumeSessionID string
	// PermissionMode sets the runtime's permission mode ("" = its default).
	// Ignored when SkipPermissions is set.
	PermissionMode string
	// AllowedTools / DisallowedTools scope the tool surface for this
	// invocation. NOTE (probed v2.1.220): path-scoped Write(...) rules do NOT
	// match in print mode — allow bare Write and enforce scope engine-side.
	AllowedTools    []string
